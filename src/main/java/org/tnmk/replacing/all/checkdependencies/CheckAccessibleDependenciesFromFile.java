...
public class CheckAccessibleDependenciesFromFile {
  ...
  public static List<CheckedDependency> missingDependencies(String repoHost, String fileName) {
    List<String> allLines = IOUtils.loadTextLinesInSystemFile(fileName);
    List<String> links = filterLinesWithDependencyLink(allLines);
    List<CheckedDependency> dependencies = links.stream().parallel().map(
        link -> CheckAccessibleDependency.isAccessibleDependency(repoHost, link)
    ).collect(Collectors.toList());
    List<CheckedDependency> missingDependencies = dependencies.stream().filter(dependency -> !dependency.isAccessible()).collect(Collectors.toList());
